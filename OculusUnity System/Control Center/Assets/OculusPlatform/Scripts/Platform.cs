  ...
  public sealed class Core {
    ...
    public static void Initialize(string appId = null)
    {
      ...
      else
      {
        if (!String.IsNullOrEmpty(PlatformSettings.AppID))
        {
          Debug.LogWarning(string.Format("The 'Oculus App Id ({0})' field in 'Oculus Platform/Platform Settings' is clobbering appId ({1}) that you passed in to Platform.Core.Init.  You should only specify this in one place.  We recommend the menu location.", PlatformSettings.AppID, appId));
        }
      }
