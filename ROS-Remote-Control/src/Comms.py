ROS_LEFT    = "left_arm_cmd"
LCM_LEFT    = "left_lcm"
LCM_LEFT_VALID = "left_lcm_valid"
ROS_LEFT_VALID = "/Baxter/left_ros_valid"
LCM_LEFT_CURRENTPOS = "left_lcm_currentpos"
ROS_LEFT_CURRENTPOS = "/Baxter/left_ros_currentpos"

ROS_RIGHT   = "right_arm_cmd"
LCM_RIGHT   = "right_lcm"
LCM_RIGHT_VALID = "right_lcm_valid"
ROS_RIGHT_VALID = "right_ros_valid"
LCM_RIGHT_CURRENTPOS = "right_lcm_currentpos"
ROS_RIGHT_CURRENTPOS = "left_ros_currentpos"

ROS_HEAD    = "head_cmd"
LCM_HEAD    = "head_lcm"

ROS_R_TRIGGER = "right_trigger_cmd"
LCM_R_TRIGGER = "right_trigger_lcm"

ROS_L_TRIGGER = "left_trigger_cmd"
LCM_L_TRIGGER = "left_trigger_lcm"

ROS_R_VEL   = "right_vel"
LCM_R_VEL   = "right_lcm_vel"
ROS_R_CMD   = "right_cmd"
LCM_R_CMD   = "right_lcm_cmd"

ROS_L_VEL   = "left_vel"
LCM_L_VEL   = "left_lcm_vel"
ROS_L_CMD   = "left_cmd"
LCM_L_CMD   = "left_lcm_cmd"

ROS_L_RANGE = "/Baxter/robot/range/left_hand_range/state"
LCM_L_RANGE = "left_lcm_range"
ROS_R_RANGE = "/Baxter/robot/range/right_hand_range/state"
LCM_R_RANGE = "right_lcm_range"

LCM_L_CAMERA = "left_lcm_camera"
ROS_L_CAMERA = "/Baxter/cameras/left_hand_camera/image"

LCM_R_CAMERA = "right_lcm_camera"
ROS_R_CAMERA = "/Baxter/cameras/right_hand_camera/image"
